...
class Seq2SeqSemanticParser(object):
    ...
    def decode(self, test_data: List[Example]) -> List[List[Derivation]]:
        ...
        # defining the listt of predictions 
        preds = []
        for index in range(len(test_data)):
            ...
            output, hidden = self.decoder.forward(SOS_embedding, torch.tensor(1).unsqueeze(0), encode_final, output_encoding_all)
            ...
            while out_index != EOS_index and i <= self.max_pred_len:
                ...
                output, hidden = self.decoder.forward(out_embedding_rep, torch.tensor([1]), hidden, output_encoding_all)
                hidden = (hidden[0].unsqueeze(0), hidden[1].unsqueeze(0))
                out_index = torch.max(output, dim=1)[1]
...
def train_model_encdec(train_data: List[Example], test_data: List[Example], input_indexer, output_indexer, args) -> Seq2SeqSemanticParser:
    ...
    # Create model
    # dimensions are 100 for the embbeding layer 
    ...
    # dimensions are 200 for the encoder and decoder and takes in the 100 dimension embedding layer
    encoder = RNNEncoder(100, 200, True)
    decoder = AttentionDecoder(100, 200, len(output_indexer))
    ...
    for epoch in range(args.epochs):
        ...
        for index in example_indices:
            ...
            # getting the output and last hidden representation
            output, hidden = decoder.forward(SOS_embedded_rep, torch.tensor([1]), encoder_final_states, encoded_output_each_word)
            ...
            for j in train_data[index].y_indexed:
                ...
                output, hidden = decoder.forward(inp_embedding_rep, torch.tensor(1).unsqueeze(0), hidden, encoded_output_each_word)
            count+=1
            loss.backward()
            optimizer.step()
