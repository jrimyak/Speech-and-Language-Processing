...
class Decoder(nn.Module):
    ...
    def forward(self, embedded_input, input_lens, hidden_input):
        ...
        h, c = hn[0][0], hn[1][0] 
        h_t = (h, c)
        # applying a linear transform to the incoming data
...
class AttentionDecoder(nn.Module):
    def __init__(self, input_size, hidden_size, num_classes):
        ...
        self.hidden_size = hidden_size
        self.rnn = nn.LSTM(input_size, hidden_size, num_layers=1, batch_first=True)
        self.lin_tran = nn.Linear(hidden_size, hidden_size*2)
        self.to_output = nn.Linear(hidden_size *3, num_classes, bias=True)
        self.init_weight()
    ...
    def init_weight(self):
        ...
        nn.init.constant_(self.rnn.bias_hh_l0, 0)
        nn.init.constant_(self.rnn.bias_ih_l0, 0)
    # following the methods in the Luong paper to compute attention
    def forward(self, embedded_input, input_lens, hidden_input, enc_output_states):
        packed_embedding = nn.utils.rnn.pack_padded_sequence(embedded_input, input_lens, batch_first=True)
        output, hn  = self.rnn(packed_embedding, hidden_input)        
        h, c = hn[0][0], hn[1][0] 
        h_t = (h, c)
        ei = torch.mm(self.lin_tran(h),enc_output_states.squeeze(1).t()) 
        ai = torch.softmax(ei,dim=1)
        c_i = torch.mm(ai,enc_output_states.squeeze(1))
